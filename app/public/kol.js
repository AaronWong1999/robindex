...
const KOLS = {
  qinbafrank: {
    ...
    library: [
      ["highlights", "过往精华", "7 个主题", "把公开内容按大主线和关键判断归档。"],
      ["sectors", "板块观点", "5 个板块", "AI、半导体、加密金融、能源电力和流动性。"],
      ["stocks", "个股档案", "82 支股票", "按公司与产业链位置整理的观点入口。"],
      ["latest", "全量库", "精选 + 720 条原文", "当前已入库最近约 4 个月公开推文。"],
    ],
    asks: [
      "AI 基建板块现在核心机会是什么？",
      "如何看待近期的市场波动？",
  ...
  aleabitoreddit: {
    ...
    library: [
      ["sectors", "供应链地图", "AI 硬件链", "沿 capex、光通信、CPO 和衬底材料拆链。"],
      ["stocks", "个股档案", "重点公司", "按 chokepoint、客户、产能和验证信号组织。"],
      ["latest", "内容库", "5,930 条原文", "GitHub 历史库 + 每日增量推文。"],
      ["research", "研究助手", "专属智能体", "用原文证据和实时行情追问。"],
    ],
    asks: [
      "AI 硬件瓶颈现在从哪里迁移到哪里？",
      "CPO 和光通信链条最该盯哪些验证指标？",
...
function nav() {
  const items = [
    ["主页", `/kol/${kol.id}`, ""],
    ["精华", `/kol/${kol.id}/highlights`, "7 个主题完整归档"],
    ["板块观点", `/kol/${kol.id}/sectors`, "5 个板块观点"],
    ["个股", `/kol/${kol.id}/stocks`, "82 票覆盖"],
    ["全量库", `/kol/${kol.id}/latest`, `${kol.id === "qinbafrank" ? "720" : "5930"} 条原文`],
...
function home() {
  const library = kol.library.map(([slug, title, meta, desc]) => {
    const href = slug === "research" ? `/research?agent=kol&persona=${kol.id}` : `/kol/${kol.id}/${slug}`;
    return `<a class="library-card" href="${href}"><span>${meta}</span><h3>${title}</h3><p>${desc}</p></a>`;
  }).join("");
      ...
      <div class="wrap kol-grid">
        ...
        <div class="kol-main">
          ...
          <section class="kol-block"><p class="eyebrow">${kol.name} 的公开学习资料和投研框架</p><div class="feature-grid">
            <div><h3>${kol.name} 观点库</h3><p>把历史公开内容按主题、公司、板块和时间线整理，追问时只引用命中的原文。</p></div>
            <div><h3>每日更新</h3><p>后台按增量抓取公开 X 内容，写入 Cloudflare D1，并保留完整原文。</p></div>
            <div><h3>AI 个股投研</h3><p>识别股票/ETF 后获取实时行情和 K 线，再结合对应 KOL 的框架回答。</p></div>
            <div><h3>专属 AI 研究助手</h3><p>每个会话固定绑定一个 KOL，persona pack 每轮注入，避免串台。</p></div>
          </div></section>
          <section class="kol-block split compact">
        ...
        <aside class="pricing-card">
          <h3>${kol.title}</h3><p>包含于 Robindex Ultra</p><div class="price">$30 <span>/月</span></div>
