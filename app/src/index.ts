      ...
      try {
        ...
        const finalMessages = [
          ...toolMsgs,
          {
            role: "user",
            content:
              "现在直接输出给用户看的最终分析。严禁输出任何工具调用、DSML、XML/HTML-like invoke/parameter/tool_calls 标签；不要说你要调用工具。" +
              "请用中文回答。保持你（博主）一贯的语气、分析框架和表达风格，像在回复读者的提问一样自然地写。" +
              "所有原文引用必须写成方括号格式，例如 [T1]、[T4]，不要写成 T1 或 #1。尽量引用 8 条以上不同原文。" +
              "区分四类信息：① 你过去直接说过的（引用原文），② 从你的框架可以推演但原文没直接提过的，③ 实时行情/新闻，④ 你的不确定性。" +
              "对每个核心判断都要回答：你过去是否直接说过、当时的上下文是什么、现在还成立需要哪些验证、什么信号会打脸。" +
