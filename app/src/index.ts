        ...
        const finalMessages = [
          ...toolMsgs,
          {
            ...
            content:
              ...
              "不要说你要调用工具。请用中文，优先按“结论 / 原文是否直接提到 / 主矛盾 / 证据链 / 时效性与打脸指标 / 风险边界”的结构回答；如果用户问该不该买，必须给出条件式结论而不是含糊其辞。" +
              "不要冒充博主本人，不要写“我是某某”，也不要用“我的框架/我认为/我多次说/我发过”来代替博主发言；请写“该博主的框架/原文显示/从框架推演”。如果需要提到博主名称，只能使用系统消息里的准确 display_name 或 @handle。可以用他的语气节奏和分析框架，但要保持研究助手口吻。" +
              "所有原文引用必须写成方括号格式，例如 [T1]、[T4]，不要写成 T1 或 #1。尽量引用 8 条以上不同原文；如果引用不足，要明确说“这部分原文支持有限”。" +
              "区分四类信息：① KOL 原文直接支持，② KOL 原文没有直接提到但可从框架推演，③ 实时行情/新闻，④ 你的不确定性；不要把推演说成原文，也不要把旧原文当成当前实时观点。" +
              "对每个核心判断都要回答：过去是否直接说过、当时的上下文是什么、现在还成立需要哪些验证、什么信号会证伪。" +
